...
class DoctorHospitalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('doctor', 'hospital_name', 'address','short_address', 'hospital_id', 'start', 'end', 'day', 'deal_price',
                  'discounted_fees', 'hospital_thumbnail', 'mrp', 'lat', 'long', 'id','enabled_for_online_booking', 'insurance',)
...
class AppointmentRetrieveSerializer(OpdAppointmentSerializer):
    ...
    def get_insurance(self, obj):
        ...
        if request:
            ...
            insured_members = user_insurance.members.all().filter(profile=obj.profile)
            if not insured_members.exists():
                resp['is_insurance_covered'] = False
                return resp

            if obj.procedures.count() > 0:
                resp['is_insurance_covered'] = False
                return resp
