...
class CouponRecommender():
        ...
        if user and user.is_authenticated:
            ...
            if cart_items:
                ...
                if payment_option_filter:
                    all_coupons.filter(Q(payment_option_id__isnull=True) \
                                     | Q(payment_option_id__isnull=False) & Q(payment_option_id=payment_option_filter.id))
    ...
    def applicable_coupons(self, **filters):
        ...
        doctor_specializations = filters.get('doctor_specializations', [])
        ...
        if lab:
            coupons = list(filter(lambda x: x.cities == None or lab.city in x.cities, coupons))
        else:
                ...
                if len(coupon.specializations.all()) == 0:
                    ...
                    if doctor_specializations:
                        ...
                        for coupon_specializations_id in coupon_specializations_ids:
                            if coupon_specializations_id in doctor_specializations:
                                keep_coupon = True
                                break
        ...
        for coupon in coupons:
            ...
            if not remove_coupon and coupon.start_date and (coupon.start_date > timezone.now() \
                    or (coupon.start_date + timedelta(days=coupon.validity)) < timezone.now()):
                remove_coupon = True
            ...
            if not remove_coupon and coupon.lab and lab and coupon.lab.id != lab.get('id'):
                remove_coupon = True

            if not remove_coupon and coupon.lab_network and lab and coupon.lab_network.id != lab.get('network_id'):
                remove_coupon = True
    ...
    def best_coupon(self, **filters):
        ...
        applicable_coupons = list(set(applicable_coupons))
