...
def update_lab_test_group_timing():
    ...
    labs_list = list()
    for lab in labs:
        # test_group_timings = lab.test_group_timings.all()
        # if not test_group_timings:
        if lab.lab_pricing_group and lab.lab_pricing_group.available_lab_tests.all():
            ...
            for data in available_lab_tests:
                lab_test_groups = LabTestGroup.objects.all()
                for lab_test_group in lab_test_groups:
                    labtest_group_mapping = LabTestGroupMapping.objects.filter(test=data.test, lab_test_group=lab_test_group).first()
                    if labtest_group_mapping:
                        lab_test_group_timing = lab.test_group_timings.filter(lab_test_group=lab_test_group).first()
                        if not lab_test_group_timing:
                            lab_timings = lab.lab_timings.all()
                            for timing in lab_timings:
                                labs_list.append(LabTestGroupTiming(lab=lab, lab_test_group=lab_test_group, day=timing.day, start=timing.start, end=timing.end,
