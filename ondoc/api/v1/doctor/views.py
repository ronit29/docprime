...
class DoctorAppointmentsViewSet(OndocViewSet):
    ...
    def create(self, request):
        serializer = serializers.CreateAppointmentSerializer(data=request.data.dict(), context={'request': request, 'data' : request.data.dict(), 'use_duplicate' : True})
        serializer.is_valid(raise_exception=True)
        validated_data = serializer.validated_data
        ...
        if validated_data.get("existing_cart_item"):
            ...
            cart_item.data = request.data.dict()
            cart_item.save()
        else:
            cart_item, is_new = Cart.objects.update_or_create(id=cart_item_id, deleted_at__isnull=True, product_id=account_models.Order.DOCTOR_PRODUCT_ID,
                                                  user=request.user,defaults={"data": request.data.dict()})
