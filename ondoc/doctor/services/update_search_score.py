...
class DoctorSearchScore:
    def __init__(self):
        ...
        try:
            for data in max_score_obj:
                if not data.score_weightage:
                    print(str(data.param) + 'weigtage not present')
                    return
                self.max_score_dict[data.param] = data.max_score
                self.scoring_data[data.param] = data.score_values
                self.weightage[data.param] = data.score_weightage.weightage
        ...
        while count <= doctors_count:
            try:
                ...
                for doctor in doctors:
                    ...
                    discount_score = self.get_discount(doctor).get('discount')
    ...
    def get_discount(self, doctor):
        try:
            discount_percentage = self.scoring_data.get('discount')
            if doctor.doctor_clinics.all():
                ...
                if clinic_time.fees and clinic_time.deal_price:
                    ...
                    for score in discount_percentage:
                        if (discount == 0):
                            return {'discount': 0}
                        if discount >= score.get('min') and not score.get('max'):
                            return {'discount': score.get('score')}
    ...
    def get_doctor_ratings(self, doctor):
        average_ratings = self.scoring_data.get('avg_rating')
        if doctor.avg_rating and doctor.avg_rating >= 1:
            for score in average_ratings:
                if doctor.avg_rating >= 4 and not score.get('max'):
    ...
    def get_practice_score(self, doctor):
        if doctor.practicing_since:
            ...
            for score in exp_years:
                if years_of_experience>=25 and not score.get('max'):
    ...
    def get_doctors_score(self, doctor_in_hosp_count,doctor):
        ...
        for score in doctors_score_list:
            if doctors_count >= 100 and not score.get('max'):
    ...
    def get_final_score(self, doctor, *args, **kwargs):
        final_score = 0
        priority_score = 0
        final_score_list = self.weightage
