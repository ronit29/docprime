...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for p in ProviderSignupLead.objects.filter(is_docprime=True, matrix_lead_id__isnull=True).order_by('-id').first():
            try:
        ...
        for h in Hospital.objects.filter(is_listed_on_docprime=True, source_type=Hospital.PROVIDER, matrix_lead_id__isnull=True).order_by('-id').first():
            try:
